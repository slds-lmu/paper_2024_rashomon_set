...
#   file.dir = "/media/external/ewaldf/TreeFARMS_perf",
#   source   = "init.R", packages = "mlr3measures"
# )
regr = loadRegistry("/media/external/ewaldf/TreeFARMS_perf", writeable = TRUE)
...
load("data/results_preds_TreeFARMS.RData")  # object: `preds_TreeFARMS`


## Build design: one row per (task, learner, model) ----------------------------

design_perf = rbindlist(
      lapply(names(preds_TreeFARMS), function(taskname) {
        n_models = length(preds_TreeFARMS[[taskname]])
        data.table(
          taskname = taskname,
...
batchExport(list(preds_TreeFARMS = preds_TreeFARMS), reg = regr)
...
res_perf_TreeFARMS = rbindlist(lapply(seq_along(res_list), function(i) {
  data.table(
